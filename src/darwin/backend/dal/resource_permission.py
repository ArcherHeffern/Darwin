...
from darwin.models.backend_models import AccountId, ResourceId, ResourcePermission as M_ResourcePermission
from darwin.backend.schemas import ResourcePermission as S_ResourcePermission


class ResourcePermissionDal(Dal_I):
    def get(self, account_id: AccountId, resource_id: ResourceId) -> Optional[M_ResourcePermission]:
        with self.db_session() as db:
            resource_permission = db.query(S_ResourcePermission).filter_by(account_id=account_id, resource_id=resource_id).one_or_none()
